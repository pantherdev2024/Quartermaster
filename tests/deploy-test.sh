#!/bin/bash
# deploy.sh is the only part of Loadout that changes anything. It is handed a
...
grep -Fq 'Loadout deployed · 2 changes' "$TRACE" || fail "wrong success notification"

# One change is reported in the singular.
run_plan '[["omarchy-theme-set","Nord"]]'
grep -Fq 'Loadout deployed · 1 change|' "$TRACE" || fail "singular not used for one change"
...
grep -Fq 'Loadout: 1 failed · omarchy-bar' "$TRACE" || fail "wrong failure notification"
